...
class ReceiptPrint implements Printable {
    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
        ...
        String encabezado = "\n\n\n\n      Avícola Río Ceballos\n"
                + "Ruta E-53 Km20-(03543) 459522\n"
                + "\n"
                + "\nFecha: " + fecha.substring(0, 10) + " Hora: " + fecha.substring(11, fecha.length() - 3) + "\n";
        String encabezado2 = """
                             Cn Producto    Precio Importe
                             """;

//        receiptDetailLine = "asdasdasda";
        Graphics2D g2d = (Graphics2D) graphics;
        Font font = new Font("Consolas", Font.BOLD, 8);
        ...
        textarea.append("" + encabezado);
        textarea.append(encabezado2);
        textarea.append(dline + "\n");

        for (ItemVentas it : items) {
            String cant = String.format("%" + (2) + "s", (int) it.cant);
            String producto = String.format("%" + (-11) + "s", it.nombre).substring(0, 11);
            String precioIndividual = String.format("%" + (6) + "s", it.precio).substring(0, 6);
            String subtotal = String.format("%" + (7) + "s", it.cant * it.precio).substring(0, 7);

            textarea.append(cant + " " + producto + " " + precioIndividual + " " + subtotal + "\n");
        }

        textarea.append(dline + "\n");
        textarea.append(String.format("%-17s", "TOTAL") + " " + String.format("%10s", formato(venta.getTotal())) + LF);
...
//        JOptionPane.showMessageDialog(null, new java.io.File("logo5.jpg").getAbsolutePath());
        while (lineCount != 0) {
            try {
            ...
            //   g2d.dr
            g2d.drawString(strText, 0, (lineNumber + 6) * 8);
...
//        font = new Font("Consolas", Font.BOLD, 8);

        Map<TextAttribute, Integer> fontAttributes = new HashMap<TextAttribute, Integer>();
//        fontAttributes.put(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_ON);
        font = new Font("Calibri", Font.BOLD, 8).deriveFont(fontAttributes);
        
        g2d.setFont(font);
        g2d.drawString("MUCHAS GRACIAS!!!", 0, (lineNumber + 6) * 8);
